...
/// BRIDGE MODE: `nado-tpm-attest.exe <64-hex-challenge> [rp-id]`
///
/// Produces a real WebAuthn `tpm` attestation statement over the chain's challenge and prints it, so it can be
/// checked by the same kernel that will judge it. It does NOT submit: nothing reaches the network from this
/// build, and the statement is bound to one sender and one landing block anyway (that is what the challenge
/// IS), so printing it gives away no ability to register anything else.
fn bridge(challenge: &[u8], rp: &str) {
    println!();
    println!("  {}", head("building an attestation statement"));
    ...
    if unsafe { NCryptOpenStorageProvider(&mut prov, pname.as_ptr(), 0) } != 0 {
        println!("  {}", bad("cannot open the chip's key store")); return;
    }
    let mut aik: NCRYPT_HANDLE = 0;
    let an = w(AIK_NAME);
    if unsafe { NCryptOpenKey(prov, &mut aik, an.as_ptr(), 0, NCRYPT_SILENT_FLAG | NCRYPT_MACHINE_KEY_FLAG) } != 0 {
        println!("  {}", bad("no identity key — run the check mode first")); return;
    }
    let mut key: NCRYPT_HANDLE = 0;
    let kn = w(KEY_NAME);
    let alg = w(BCRYPT_RSA_ALGORITHM);
    if unsafe { NCryptCreatePersistedKey(prov, &mut key, alg.as_ptr(), kn.as_ptr(), 0, NCRYPT_OVERWRITE_KEY_FLAG) } != 0 {
        println!("  {}", bad("the chip would not create a key")); return;
    }
    let len: u32 = 2048;
    let lp = w(NCRYPT_LENGTH_PROPERTY);
    unsafe { NCryptSetProperty(key, lp.as_ptr(), &len as *const u32 as *const u8, 4, 0) };
    if unsafe { NCryptFinalizeKey(key, 0) } != 0 {
        println!("  {}", bad("the chip would not finalize the key")); return;
    }

    // The credential key's public area has to be known BEFORE the claim, because authData embeds it and the
    // nonce is a hash over authData. One claim is made only after everything it commits to is fixed.
    let probe = match unsafe { make_claim(key, aik, None) } { Ok(b) => b, Err(rc) => {
        println!("  {}", bad(&format!("first claim failed 0x{rc:08x}"))); return; } };
    let pub_area = match claim_pub_area(&probe) { Some(p) => p.to_vec(), None => {
        println!("  {}", bad("could not find the key's public area in the claim")); return; } };
    let (modulus, exponent, _) = match rsa_from_pub_area(&pub_area) { Some(v) => v, None => {
        println!("  {}", bad("public area is not an RSA key we can read")); return; } };

    let cose = cose_rsa(&modulus, exponent);
    let cred_id = sha256(&pub_area);
    ...
    let cdj = format!(
        ...
        b64url_nopad(challenge), rp);
    let cdj_hash = sha256(cdj.as_bytes());

    // extraData the verifier will recompute: hash(authData || sha256(clientDataJSON)), with the hash implied
    // by `alg`. RS256 -> SHA-256.
    let mut signed = ad.clone();
    signed.extend_from_slice(&cdj_hash);
    let nonce = sha256(&signed);

    let claim = match unsafe { make_claim(key, aik, Some((&nonce, NCRYPTBUFFER_CLAIM_KEYATTESTATION_NONCE))) } {
        Ok(b) => b,
        Err(rc) => { println!("  {}", bad(&format!("claim with nonce failed 0x{rc:08x}"))); return; }
    };
    let (cert_info, sig) = match claim_certify(&claim, &nonce) {
        Some(v) => v,
        None => { println!("  {}", bad("the chip did not bind our challenge into the certify")); return; }
    };
    let pub_area2 = claim_pub_area(&claim).map(|p| p.to_vec()).unwrap_or(pub_area);

    let x5c = unsafe { aik_cert_der(aik) };
    let att = attestation_object(-257, &x5c, &sig, &cert_info, &pub_area2, &ad);

    println!("  {}  certInfo {} · sig {} · pubArea {} · authData {} · x5c {}",
             ok("built"), cert_info.len(), sig.len(), pub_area2.len(), ad.len(),
             if x5c.is_empty() { "MISSING".to_string() } else { format!("{} bytes", x5c[0].len()) });
    if x5c.is_empty() {
        println!("  {}", warn("no AIK certificate on this PC, so the chain cannot verify it yet —"));
        println!("  {}", warn("everything else below is real and checkable."));
    }
    // the AIK public key, so the signature can be verified without the certificate
    if let Some(p) = unsafe { export_pub(aik, BCRYPT_RSAPUBLIC_BLOB) } {
        println!("\n  aikpub={}", b64(&p));
    }
    println!("\n  att={}", b64(&att));
    println!("\n  cdj={}", b64(cdj.as_bytes()));
    println!("\n  rp={rp}");

    unsafe { NCryptDeleteKey(key, 0) };
    unsafe { NCryptFreeObject(aik) };
    unsafe { NCryptFreeObject(prov) };
}

/// The credential key's public area out of a claim: the PCP blob's TPM2B_PUBLIC.
fn claim_pub_area(b: &[u8]) -> Option<&[u8]> {
    let at = b.windows(4).position(|w| w == b"PCPM")?;
    let kb = &b[at..];
    let u = |o: usize| -> Option<usize> {
        Some(u32::from_le_bytes([*kb.get(o)?, *kb.get(o+1)?, *kb.get(o+2)?, *kb.get(o+3)?]) as usize)
    };
    let (hdr, pubn) = (u(4)?, u(16)?);
    kb.get(hdr..hdr + pubn)
}

/// (certInfo, signature) from the KADS section — located by the CERTIFY attest that carries our nonce, so a
/// container change cannot silently hand back the wrong bytes.
fn claim_certify(b: &[u8], nonce: &[u8]) -> Option<(Vec<u8>, Vec<u8>)> {
    let off = find_attests(b).into_iter()
        .find(|&o| matches!(attest_at(b, o), Some((0x8017, _, ref e)) if e == nonce))?;
    let kads = b[..off].windows(4).rposition(|w| w == b"KADS")?;
    let u = |o: usize| u32::from_le_bytes([b[o], b[o+1], b[o+2], b[o+3]]) as usize;
    let (ci, sg) = (u(kads + 12), u(kads + 16));
    if kads + 24 != off { return None; }                      // certInfo must start right after the header
    Some((b.get(off..off + ci)?.to_vec(), b.get(off + ci..off + ci + sg)?.to_vec()))
}

/// The DER of the AIK's certificate, matched by its issuer naming a TPM-vendor KeyId.
unsafe fn aik_cert_der(_aik: NCRYPT_HANDLE) -> Vec<Vec<u8>> {
    let store_name = w("My");
    let store = CertOpenStore(CERT_STORE_PROV_SYSTEM_W, 0, 0, CERT_SYSTEM_STORE_LOCAL_MACHINE,
                              store_name.as_ptr() as *const _);
    let mut out = Vec::new();
    if store.is_null() { return out; }
    let mut ctx: *const CERT_CONTEXT = std::ptr::null();
    loop {
        ctx = CertEnumCertificatesInStore(store, ctx);
        if ctx.is_null() { break; }
...
pub fn main() {
    ...
    if let Some(hexchal) = args.first() {
        if hexchal.len() == 64 && hexchal.chars().all(|c| c.is_ascii_hexdigit()) {
            let chal: Vec<u8> = (0..32).map(|i| u8::from_str_radix(&hexchal[i*2..i*2+2], 16).unwrap()).collect();
            let rp = args.get(1).cloned().unwrap_or_else(|| "get.nadochain.com".into());
            unsafe { COLOUR = enable_colour() };
            if !is_elevated() {
                println!("  Run this as administrator."); pause(); return;
            }
            bridge(&chal, &rp);
            pause();
            return;
