...
def get_transaction_pool_demo():
    ...
    ip = config["ip"]
    port = config["port"]
    tx_pool_message = requests.get(f"http://{ip}:{port}/transaction_pool?compress=msgpack", timeout=10).text
...
def get_block_producers_hash_demo():
    ...
    ip = config["ip"]
    port = config["port"]
    status_message = requests.get(f"http://{ip}:{port}/status", timeout=10).text
...
if __name__ == "__main__":
    ...
    for _ in range(0, no_of_blocks):
        ...
        requests.get(f"http://{ip}:{port}/submit_block?data={json.dumps(block_message)}&key={server_key}", timeout=10)