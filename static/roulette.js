...
async function fetchTable(t) { const sto = await dapp.storage({ append: ["gd", "gr", "gw", "gg", "cov"] }); return sto ? tableFrom(sto, t) : null; }
...
async function refreshActive() {
  ...
  const sto = await dapp.storage({ append: ["gd", "gr", "gw", "gg", "cov"] });
  if (sto) {
    lastSto = sto;
    pruneAndTrack(sto);
    if (activeTable != null) {
