...
//      so NOTHING here is a float. Positions and velocities are Q10 integers; the only division is
//      Math.trunc on products that provably stay under 2^53; square roots are an exact integer isqrt
//      (Newton-seeded, then corrected until r² ≤ n < (r+1)², so a differently-rounded Math.sqrt cannot
//      change the answer); and the sine table is built once from a BigInt Taylor series over a single
//      hard-coded π/2 constant — Math.sin/Math.cos are NOT specified to be identical across engines and
//      are never called. Two clients replaying the same log therefore rack up the same table, ball for
//      ball, to the unit.
//
...
// ---- table geometry (units ≈ mm on a 9-foot table); all state is Q10 fixed point --------------------
...
export const POCKET_MOUTH = POCKET_RS.map((r) => Math.floor(Math.sqrt(r * r - R * R)));
...
const SPIN_K = 40;                            // spin → acceleration coupling (Q10 numerator over 1024)
...
function pocketed(p, k) {
  const dx = Math.trunc(p.x / FP) - POCKETS[k][0], dy = Math.trunc(p.y / FP) - POCKETS[k][1];
...
export const placeToXY = (px, py) => [R + Math.trunc((px * (W - 2 * R)) / 2047), R + Math.trunc((py * (TH - 2 * R)) / 1023)];
...
function rackOrder(q) {
  ...
  // slots 0..14 in row-major order; slot 4 (row 2, middle) is the 8
...
export function placeCue(st, x, y, kitchen) {
  const lo = kitchen ? R : R, hi = kitchen ? HEAD_X - R : W - R;
...
// simulate(st, shot): run one shot to a complete stop. Mutates the ball array and returns the event
// record the ruleset judges, plus the animation frames (cosmetic — never fed back into state).
export function simulate(st, shot, wantFrames, wantPaths) {
  ...
  for (let tick = 0; tick < MAX_TICKS; tick++) {
    ...
    // An aim line is only useful up to the point it tells you something. Recording the WHOLE journey drew
    // the cue ball ricocheting off four cushions and the object ball wandering the length of the table —
    // spaghetti across the cloth that hid the one line the player cares about. Cut both short: the cue up
    // to its first contact (plus a little carry so you can read the angle it leaves at), and the object
    // ball until it reaches a cushion.
...
export function applyShot(st, side, shot, wantFrames) {
  ...
  for (const p of ev.pots) {
    if (p.ball === 8) eightPot = p;
      ...
      if (wasOpen) potMine++;                       // an open table credits whatever fell (group is claimed below)
  ...
  if (!over && ev.cueScratch === false && !st.b[0].on) placeCue(st, trunc(HEAD_X / 2), FOOT_Y, false);
  ...
  if (over) { st.over = true; st.result = winner + 1; }
    ...
    st.inHand = !!foul;
    if (!keep) st.turn = 1 - side;
    else st.turn = side;
    if (foul) st.turn = 1 - side;
  }
  st.breakShot = false;
  st.broke = true;
  const rec = { side, shot, foul, pots: ev.pots.slice(), first: ev.first, keep, over, winner: over ? winner + 1 : 0 };
  st.log.push(rec);
  st.last = { rec, frames };
  return st.last;
}
...
export function ghostAim(st, ballId, pocketIdx) {
  ...
  if (ad === 0) return null;
  // cut angle via the dot product of the cue path and the object path (Q16 cosine)
  const cut = trunc((ax * tx + ay * ty) * 65536 / (ad * td));
  return { angle: angleOf(ax, ay), dist: ad, potDist: td, cut };
}
// pathClear(st, fromX, fromY, toX, toY, ignore): is the straight line free of other balls?
...
export function botShot(st, side, rnd) {
  ...
  for (const id of targets) {
    for (let k = 0; k < 6; k++) {
      ...
      const score = aim.cut - Math.trunc((aim.dist + aim.potDist) * 8);
  ...
  return { op: 1, angle: near.angle, power: Math.max(8, Math.min(40, 10 + Math.trunc(isqrt(near.d) / 80))),
           side: 0, fwd: 0, call: 7, px: 0, py: 0, _place: st.inHand };
}
