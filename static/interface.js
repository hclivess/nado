...
function deviceGuide(st) {
  ...
  if (isWin && (fmt === "none" || !st.x5c)) return i18("device.guide.winAik",
    ...
    "If ManufacturerCertificates is EMPTY, the chip never got an endorsement certificate and this PC cannot attest through Windows Hello at all. If it is PRESENT and certreq still returns 404, the certificate exists but Windows cannot present it to Microsoft under a certificate authority Microsoft knows — typically because the issuing chain above it (AdditionalCertificates, empty on the machine that produced this note) was never installed and cannot be fetched. Install the OEM's latest BIOS and its Intel Management Engine / CSME (or AMD chipset) firmware package, reboot, and run certreq again: that is what re-provisions the endorsement chain. Either way, do NOT clear the TPM to chase this — it destroys BitLocker recovery material and does not restore a missing chain. If instead certreq times out or cannot resolve the host, the PC simply cannot reach Microsoft's AIK service: try another network, such as a phone hotspot, and run it again. While the PC is like this, Attest from another device works today and costs nothing.\n\n" +
