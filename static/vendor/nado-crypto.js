// nado-crypto.js — vendored ESM crypto bundle for the browser wallet/miner/messaging.
// Exports: blake2b, bytesToHex, hexToBytes, ml_dsa44 (ML-DSA-44 / FIPS 204), ml_kem768 (ML-KEM-768 / FIPS 203).
// GENERATED by esbuild — do NOT hand-edit. Rebuild (doc/messaging.md §10):
//   npm i @noble/hashes@1.4.0 @noble/post-quantum@0.2.0 esbuild
//   printf '%s\n' \
//     "export { blake2b } from '@noble/hashes/blake2b';" \
//     "export { bytesToHex, hexToBytes } from '@noble/hashes/utils';" \
//     "export { ml_dsa44 } from '@noble/post-quantum/ml-dsa';" \
//     "export { ml_kem768 } from '@noble/post-quantum/ml-kem';" > entry.js
//   npx esbuild entry.js --bundle --format=esm --outfile=static/vendor/nado-crypto.js
// ml_dsa44 pubkeys are byte-identical to the node's dilithium-py ML-DSA-44 (verified seed 0x07*32 -> 224c6192...).
...
var BLAKE = class extends Hash {
  constructor(blockLen, outputLen, opts2 = {}, keyLen, saltLen, persLen) {
    super();
    ...
    if (opts2.key !== void 0 && (opts2.key.length < 1 || opts2.key.length > keyLen))
      throw new Error(`key must be up 1..${keyLen} byte long or undefined`);
    if (opts2.salt !== void 0 && opts2.salt.length !== saltLen)
      throw new Error(`salt must be ${saltLen} byte long or undefined`);
    if (opts2.personalization !== void 0 && opts2.personalization.length !== persLen)
...
var BLAKE2b = class extends BLAKE {
  constructor(opts2 = {}) {
    super(128, opts2.dkLen === void 0 ? 64 : opts2.dkLen, opts2, 64, 16, 16);
    this.v0l = B2B_IV[0] | 0;
    this.v0h = B2B_IV[1] | 0;
    this.v1l = B2B_IV[2] | 0;
    this.v1h = B2B_IV[3] | 0;
    ...
    const keyLength = opts2.key ? opts2.key.length : 0;
    this.v0l ^= this.outputLen | keyLength << 8 | 1 << 16 | 1 << 24;
    if (opts2.salt) {
      const salt = u32(toBytes(opts2.salt));
      this.v4l ^= byteSwapIfBE(salt[0]);
      this.v4h ^= byteSwapIfBE(salt[1]);
      this.v5l ^= byteSwapIfBE(salt[2]);
      this.v5h ^= byteSwapIfBE(salt[3]);
    }
    if (opts2.personalization) {
      const pers = u32(toBytes(opts2.personalization));
...
var blake2b = /* @__PURE__ */ wrapConstructorWithOpts((opts2) => new BLAKE2b(opts2));
...
var polyCoder = (d, compress2) => bitsCoder(d, {
  encode: (i) => compress2 ? compress2(i) : i,
  decode: (i) => compress2 ? compress2(i) : i
});
var polyAdd = (a, b) => {
  for (let i = 0; i < a.length; i++)
    a[i] = mod(a[i] + b[i]);
