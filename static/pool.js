...
import * as E from "./pool-engine.js?v=54b81f0c";
...
let anim = null;                                   // { frames, i, per } — the shot currently playing back
...
const duel = new DuelGame(dapp, {
  ...
  overHint(eng, me) {
    ...
    if (!last) return "";
    const eight = last.pots.some((p) => p.ball === 8);
    if (!eight) return "";
    const winner = eng.result - 1;
    return winner === me ? T("hintYouSank8", "You sank the 8 in the called pocket.")
...
function drawTable(eng, gm) {
  ...
  g.strokeStyle = "rgba(0,0,0,.35)"; g.lineWidth = Math.max(1, sc); g.strokeRect(tx(0), ty(0), E.W * sc, E.TH * sc);
  ...
  if (live && balls[0].on && assist > 0) {
    const c = balls[0], pv = previewFor(eng);
...
function renderGame(gm, eng) {
  ...
  $("hud").innerHTML =
    ...
    + (eng.inHand && eng.turn === me ? '<span class="stat" style="border-color:#00c9a7;color:#00c9a7">' + T("ballInHand", "BALL IN HAND — drag the cue ball") + "</span>" : "");
  ...
  $("powerRange").disabled = readOnly;
  for (const id of ["btnAimL10", "btnAimL1", "btnAimR1", "btnAimR10", "btnSpinClear"]) {
    const b2 = $(id); if (b2) b2.disabled = readOnly;
  }
  if (!showBar) return;
...
//    played at wildly different rates from the same simulation. Playback is now driven by the WALL CLOCK
//    at a fixed frames-per-second, so a long shot takes longer than a short one, exactly as it should,
//    and a slow device gets the same duration (just choppier) instead of a slow-motion replay.
...
// queueShot(mi, frames, side): hand one shot to the player, in order. `mi` dedupes — the engine is
...
function onPointer(ev, phase) {
  ...
  if (dragging === "cue") {
    ...
    const kitchen = eng.breakShot;
    E.placeCue(eng, ux, uy, kitchen);
...
if (typeof window !== "undefined") {
  window.__duel = duel;
  window.__pool = { aim, E, busy: () => !!(anim || animQ.length), queued: () => animQ.length,
                    assist: () => assist, setAssist, side: () => (anim ? anim.side : null), playLog,
                    shown: () => (duel.eng ? shownBalls(duel.eng) : null),
                    at: () => (anim ? [anim.i, anim.frames.length, anim.per] : null) };
}
