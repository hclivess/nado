#!/usr/bin/env bash
# fire_cutover.sh — EXECUTE the betanet-7 debrand reroll. Run ON THE OPERATOR NODE, from the repo
...
#   (anything not moved does NOT carry to betanet-7)
#
...
# CHAIN_GENERATION bump, purge, and boot betanet-7 from block 1 with balances pre-seeded).
set -euo pipefail
cd "$(dirname "$0")/.."
...
NOW=$(date +%s)
sed -i "s/^GENESIS_TIMESTAMP = .*/GENESIS_TIMESTAMP = ${NOW}  # betanet-7 — the debrand cutover reroll/" protocol.py
echo "== 4/5 commit + push"
git add protocol.py genesis_data/genesis_alloc.dat genesis_data/genesis_open.dat
git commit -m "FIRE: betanet-7 debrand reroll — genesis $(date -u -d @${NOW} +%FT%TZ), balances re-keyed to ${NEW_PREFIX}"
...
echo; echo "DONE — watch /status: nodes purge + boot betanet-7 from block 1. (185.184.192.210 has no systemd: its operator must restart the process.)"
