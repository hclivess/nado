...
if __name__ == "__main__":
    ...
    for x in range(0, 50000):
        try:
            ...
            requests.get(f"http://{ip}:{port}/submit_transaction?data={json.dumps(transaction)}", timeout=10)
        except Exception as e:
            print(e)

    # tx_pool = json.loads(requests.get(f"http://{ip}:{port}/transaction_pool").text, timeout=10)
