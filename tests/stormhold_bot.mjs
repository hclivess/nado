/*
 * stormhold_bot.mjs — a legal-move generator over the REAL engine (shared by the fuzz test and the live
 * on-chain E2E): given a replayed state it produces one legal move for the current actor, mirroring what
 * the UI would allow. Kept separate so the live smoke test plays the same rules the browser enforces.
 */
