...
The gate (SHIELD_WIDE_HEIGHT) was 2^62 on gen 25 and 1 from gen 26; it was deleted after the betanet-8 reroll, so the
wide pool is the pool from block 1 and only height 0 (genesis, which carries no transactions) is still below it.
Properties:
  * an empty state's root and snapshot are byte-unchanged by the wide pool's existence (empty is absent);
  * a `shield` deposit lands in the WIDE pool as commit(amount, owner, rho) over digests, and L1 admission refuses a
    deposit whose owner is not a 64-hex digest (coins would otherwise be escrowed behind a note the exec layer never
    creates); at height 0 the legacy pool takes it, as below the deleted gate;
  ...
  * a joinsplit2 bundle is refused from block 1 and a joinsplit3 bundle is refused at height 0;
...
def t_empty_root_and_snapshot_unchanged_by_the_wide_pool():
    st = _state()
    assert not hasattr(protocol, "SHIELD_WIDE_HEIGHT"), "the gate is deleted: the wide pool is the pool from block 1"
    assert not st.rules_shield_wide(), "a fresh state (cursor -1) applies genesis, below the deleted gate"
    ...
    # a legacy deposit at height 0 (below the deleted gate) lands in the legacy pool, exactly as before
    _apply(st, 0, [_shield_tx(1000, str(alghash.owner_of(NSK_A)), 7)])
...
def t_deposit_lands_in_the_wide_pool():
    st = _state()
...
def t_l1_admission_pins_the_owner_shape():
...
def t_bundle_kinds_flip_at_the_gate():
    # a joinsplit2 bundle from block 1, a joinsplit3 bundle at height 0: both refused without running a proof
    st = _state()
    st._applying = H
