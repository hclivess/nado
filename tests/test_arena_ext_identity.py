...
def run(bk, rc):
    """Prove natively. NADO_STRICT_NATIVE makes a fallback an ERROR, so a green run cannot be the Python
    path wearing a native label — the exact mislabelling that hid the compose_ext alpha bug for days."""
    env = dict(os.environ)
...
for label, bk, rc in (("col-commit / DEFAULT", BK.DEFAULT, False),
    ...
    # ...and the verifier is not merely permissive: perturb a boundary the proof does NOT satisfy.
    bad = [(0, 3, 12), (T - 1, 3, 11)]
    ok2, _ = stark.verify(p_nat, cons, bad, periodic=per, max_degree=3, num_queries=12,
                          aux_spec=aux_spec, row_commit=rc, backend=bk)
    check(not ok2, f"{label}: a proof checked against a DIFFERENT boundary is rejected")

print()
if fails:
    print(f"{len(fails)} FAILED"); sys.exit(1)
print("ARENA PRODUCES VERIFYING GF(p^%d) PROOFS IN BOTH COMMIT MODES" % D)
