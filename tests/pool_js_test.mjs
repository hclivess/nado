 ...
 * escape the table or overlap at rest; and the 8-ball rules (break legality, group assignment, first-
 * contact / no-rail / scratch fouls, ball in hand, the called-pocket ending) fire where they should.
...
const { W, TH, R, FP, BALLS, POCKETS, POCKET_R, HEAD_X, FOOT_Y, isqrt, SIN, COS, angleOf,
        encShot, decShot, init, applyMove, applyShot, replay, simulate, placeCue, botEnc,
        legalTargets, targetGroup, remaining, groupOf, placeToXY, xyToPlace, V_MIN, V_STEP } = E;
...
check("the trig table is a unit circle to Q16 and matches Math.sin closely enough to be right", () => {
  for (let a = 0; a < 4096; a++) {
    ...
    // |(sin²+cos²) − 1| must be within the Q16 quantisation of both components
    if (Math.abs(m - 65536 * 65536) > 4 * 65536) throw new Error(`a=${a} magnitude off: ${m / (65536 * 65536)}`);
    const want = Math.sin((a * 2 * Math.PI) / 4096) * 65536;
    if (Math.abs(SIN[a] - want) > 2) throw new Error(`SIN[${a}] = ${SIN[a]} vs ${want}`);
    const wantC = Math.cos((a * 2 * Math.PI) / 4096) * 65536;
    if (Math.abs(COS[a] - wantC) > 2) throw new Error(`COS[${a}] = ${COS[a]} vs ${wantC}`);
  }
  // the table is built from BigInt integers only — it must be reproducible bit for bit
...
check("shot encoding round-trips every field at full width", () => {
  ...
  for (let i = 0; i < 3000; i++) {
    ...
    for (const k of Object.keys(s)) if (d[k] !== s[k]) throw new Error(`${k}: ${s[k]} -> ${d[k]} (enc ${enc})`);
  }
  // the widest legal encoding must still be JSON-safe (the exec view decodes slots as JS numbers)
  const maxEnc = encShot({ angle: 4095, power: 63, side: 3, fwd: 3, call: 7, px: 2047, py: 1023 });
  if (!Number.isSafeInteger(maxEnc) || maxEnc >= 2 ** 53) throw new Error("max enc " + maxEnc + " overflows");
});
...
check("placement grid maps into the legal cue-ball rectangle and back", () => {
  for (let px = 0; px < 2048; px += 37) for (let py = 0; py < 1024; py += 29) {
    ...
    if (x < R || x > W - R || y < R || y > TH - R) throw new Error(`placeToXY(${px},${py}) = ${x},${y} off table`);
...
check("the rack is seed-determined, complete and legal", () => {
  ...
  // different seeds must actually produce different racks (the shuffle is wired up)
  const racks = new Set();
...
check("the 8 sits in the centre of the third row and the back corners are one of each", () => {
  for (let s = 0; s < 60; s++) {
    const st = init(1, qOf(s));
    // slot 4 = row 2 middle; slots 10 and 14 = the two back corners
    const at = (row, k) => {
...
check("a shot is a pure function of (state, shot) — identical twice, identical on replay", () => {
  ...
  for (let t = 0; t < 12; t++) {
    ...
    const enc = encShot(shot);
    const c = replay(5, qOf(t), [{ enc, side: 1 }]);
...
check("balls stay on the table, never overlap at rest, and always come to a stop", () => {
  ...
  for (let t = 0; t < 30; t++) {
    ...
    for (let s = 0; s < 8 && !st.over; s++) {
      ...
      applyShot(st, st.turn, shot, false);
      for (let i = 0; i < BALLS; i++) {
        const p = st.b[i];
        if (p.vx !== 0 || p.vy !== 0) throw new Error("ball " + i + " still moving after the shot resolved");
        if (!p.on) continue;
        const x = p.x / FP, y = p.y / FP;
        if (x < R - 1 || x > W - R + 1 || y < R - 1 || y > TH - R + 1) throw new Error(`ball ${i} left the table at ${x},${y}`);
      }
      for (let i = 0; i < BALLS; i++) for (let j = i + 1; j < BALLS; j++) {
...
check("shot power maps to a sane roll distance", () => {
  const roll = (power) => {
    ...
    for (let i = 1; i < BALLS; i++) st.b[i].on = false;          // clear the table: measure the cue ball alone
    st.b[0].x = 200 * FP; st.b[0].y = FOOT_Y * FP;
    st.inHand = false; st.breakShot = false;
...
check("draw and follow move the cue ball in opposite directions off a straight pot", () => {
  ...
  if (!(draw < stun)) throw new Error(`backspin must pull the cue ball back: draw ${draw} vs stun ${stun}`);
  if (!(follow > stun)) throw new Error(`topspin must push it forward: follow ${follow} vs stun ${stun}`);
});
...
check("side spin bends the cue ball's path", () => {
  const endY = (side) => {
    ...
    for (let i = 1; i < BALLS; i++) st.b[i].on = false;
    st.b[0].x = 200 * FP; st.b[0].y = FOOT_Y * FP;
    simulate(st, { angle: 0, power: 10, side, fwd: 0 }, false);   // gentle: it must stop short of the far rail
...
check("a pocket accepts by DIRECTION, not mere proximity", () => {
  // a ball running along the top rail must sail PAST the side pocket, not be swallowed by it
  ...
  // but squared up at it, the same pocket takes it
  const into = posed([[0, W / 2, 600], [1, 2400, 1100]]);
  const hit = simulate(into, { angle: E.angleOf(0, -1), power: 30, side: 0, fwd: 0 }, false);
  if (!hit.ev.pots.some((p) => p.ball === 0 && p.pocket === 1))
    throw new Error("a ball played straight into the side pocket did not drop");
  // corners are forgiving: a rail-runner heading INTO the corner still goes down
  const corner = posed([[0, 600, R + 1], [1, 2400, 1100]]);
  const cp = simulate(corner, { angle: E.angleOf(-1, 0), power: 30, side: 0, fwd: 0 }, false);
...
// ---- 5. the 8-ball ruleset ---// ---- 5. the 8-ball ruleset ---------------------------------------------------------------------------
// aim the cue ball straight at a point and shoot
...
check("scratching is a foul even on an otherwise good pot", () => {
  // cue ball straight into a corner pocket with nothing else near it
...
check("potting the 8 early loses the game", () => {
  ...
  if (!st.over || st.result !== 2) throw new Error(`potting the 8 with balls left must lose: over=${st.over} result=${st.result}`);
});
...
check("the 8 in the called pocket wins; the wrong pocket loses", () => {
  ...
  if (!win.over || win.result !== 1) throw new Error(`called-pocket 8 must win: over=${win.over} result=${win.result}`);
  const lose = mk();
...
check("scratching while potting the 8 loses", () => {
  // the cue ball is straight behind the 8 on the pocket line, hit hard with follow — both go down
  const st = posed([[0, 400, 400], [8, 300, 300], [9, 1500, 635]]);
  const { rec } = shootAt(st, 0, POCKETS[0][0], POCKETS[0][1], 63, 0, 3);
  if (!rec.pots.some((p) => p.ball === 8) || !rec.pots.some((p) => p.ball === 0)) throw new Error("expected both the 8 and the cue ball down, got " + JSON.stringify(rec.pots));
...
check("the aim preview is the shot: it never promises a contact the physics does not make", () => {
  ...
  for (let t = 0; t < 120; t++) {
    ...
    // scatter the rack a little so the positions are varied, then compare preview vs the real shot
    applyShot(st, 0, { op: 1, angle: 0, power: 40 + (t % 20), side: 0, fwd: 0, call: 7,
                       ...(() => { const [px, py] = xyToPlace(Math.round(HEAD_X / 2), FOOT_Y); return { px, py }; })() }, false);
    for (let k = 0; k < 4 && !st.over; k++) {
      ...
      if (pre.first !== rec.first) throw new Error(`preview said first contact ${pre.first}, the shot made ${rec.first}`);
      ...
      if (pre.first >= 0 && (!pre.cuePath || pre.cuePath.length < 2)) throw new Error("a contact with no cue path to draw");
    }
  }
...
check("the preview line stops where it stops being useful", () => {
  // a shot that contacts: the cue line must reach the contact point, and not run on round the table
  ...
  const last = [pv.cuePath[pv.cuePath.length - 2], pv.cuePath[pv.cuePath.length - 1]];
  const dx = last[0] - pv.firstAt[0], dy = last[1] - pv.firstAt[1];
  if (isqrt(dx * dx + dy * dy) > 420) throw new Error("the cue line runs far past the contact point");
  // and the object line is bounded, however hard the ball is struck
  let run = 0;
  ...
  // a shot that contacts AFTER a cushion must still draw the line all the way to the contact
  const bank = posed([[0, 1200, 200], [1, 1200, 1000], [9, 300, 300]]);
  const bp = E.previewShot(bank, { angle: E.angleOf(0, -1), power: 55, side: 0, fwd: 0, call: 7, px: 0, py: 0 });
  if (bp.first >= 0) {
    const l2 = [bp.cuePath[bp.cuePath.length - 2], bp.cuePath[bp.cuePath.length - 1]];
    const ex = l2[0] - bp.firstAt[0], ey = l2[1] - bp.firstAt[1];
    if (isqrt(ex * ex + ey * ey) > 420) throw new Error("a banked contact was not drawn through to the contact");
  }
});

check("English curves the cue ball away from the straight line a ray-cast would draw", () => {
  // the exact case the old straight-line preview got wrong: aim so a naive ray hits, then add side spin
