 ...
 * state → payload encoding → applyMove → repaint — plus the free-frame toggle and the ball-in-hand drag.
 *
...
try {
  ...
  await scenario("a zero stake is the free frame", async () => {
    const set = (v) => evl(`(() => { const i = document.getElementById("stakeAmt");
      i.value = ${JSON.stringify(v)}; i.dispatchEvent(new Event("input")); return i.value; })()`);
    await set("1");
    ...
    // the SDK slider (injected next to the input when signed in) drives the same value
    const hasSlider = await evl(`!!document.getElementById("stakeSlider")`);
    assert(hasSlider, "the SDK stake slider was not injected next to the stake input");
    await set("1");
    assert(await evl(`window.__duel.amt(0n).length > 0 && window.__duel.amt(0n) !== "0 NADO"`),
           "a zero pot must render as 'free', not '0 NADO'");
  });
  ...
  await scenario("power and English controls feed the shot payload", async () => {
    await evl(`(() => { const r = document.getElementById("powerRange"); r.value = "63";
    ...
    const spun = await aimState();
    assert(spun.fwd > 0, "tapping the top of the cue-ball widget should set top spin, got " + spun.fwd);
  ...
  await scenario("arming and firing plays the break, and the bot answers", async () => {
    ...
    assert(after.onTable <= 16, "balls appeared from nowhere");
  });
  ...
  await scenario("the break animates the whole rack, not one ball", async () => {
    ...
    // aim straight down the table at the rack apex and hit it as hard as it goes
    await evl(`(() => { const e = window.__duel.eng, E = window.__pool.E;
      let apex = 1, ax = 1e9;
    ...
    // sample the DRAWN positions across the animation and count how many balls actually move
    const moved = new Set();
  ...
  await scenario("playback is real-time: duration tracks the shot and matches the wall clock", async () => {
    ...
    const long = log.reduce((a, b) => (b.n > a.n ? b : a));
    const short = log.reduce((a, b) => (b.n < a.n ? b : a));
    ...
    await evl(`document.getElementById("btnPractice").click()`);
    await sleep(400);
    await evl(`document.getElementById("btnPractice").click()`);   // interrupt again for good measure
    await sleep(1200);
    ...
    const scheduled = played.reduce((a, b) => a + b, 0);
    // too QUICK means something is stepping the animation on beyond the clock (the old stale-loop bug);
    // too SLOW means duration is following the frame rate instead of the clock (the old counter bug).
  ...
  // Driven until the opponent ACTUALLY replies — potting keeps you at the table, so a single shot is not
  // enough to guarantee they ever get one.
  await scenario("the opponent's shot replays with their own instruments", async () => {
    ...
    // a no-contact foul and passes the turn. Hoping a normal shot misses is how this test flaked — potting
    ...
    await evl(`(() => { const r = document.getElementById("powerRange"); r.value = "0";
                        r.dispatchEvent(new Event("input")); })()`);
    await shoot();
    for (let i = 0; i < 200; i++) {
      await sleep(150);
  ...
  await scenario("the aim assist slider turns the preview off, and nothing lingers into the shot", async () => {
    await waitStill();
    if (await evl(`!!(window.__duel.eng && window.__duel.eng.over)`)) {
      await evl(`document.getElementById("btnPractice").click()`); await sleep(1500);
    }
    ...
    // the slider reflects it, and the setting survives a re-render
  ...
  await scenario("a full practice frame plays out through the UI without corrupting", async () => {
    for (let i = 0; i < 25; i++) {
      ...
      // aim at whatever the engine offers, else just poke down-table
      const picked = await evl(`(() => { const b = document.querySelector('#potBtns [data-pot]');
        if (b) { b.click(); return true; } return false; })()`);
      if (!picked) await drag([200, 300], [2400, 900]);
      await evl(`(() => { const r = document.getElementById("powerRange"); r.value = "34";
                          r.dispatchEvent(new Event("input")); })()`);
  ...
  await scenario("whenever it is your shot, the instrument is actually there", async () => {
    ...
    const check = (s, where) => {
      if (s.over) return;
      // the badge must never claim the turn while the controls are gated shut
    ...
    // play a few shots, sampling the invariant right through the animations
    for (let i = 0; i < 3; i++) {
