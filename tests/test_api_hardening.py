...
def t1_untrusted_peer_ignores_xff():
    """Prove a forged X-Forwarded-For from an untrusted direct peer is ignored (peer IP wins)."""
...
def t1_trusted_proxy_uses_xff():
    """Prove XFF is honored when the peer is a configured trusted proxy (empty XFF falls back to peer)."""
...
def t1_walks_past_chained_proxies():
    """Prove chained trusted proxies resolve to the leftmost untrusted hop, never a trusted IP."""
...
def t2_check_ip_rejects_ssrf_targets():
    """Prove check_ip rejects non-routable/internal/malformed force-sync targets and accepts a global IP."""
