...
def _canonical_pre_contracts(pre, fixed_names=False):
    ...
    other name. Keep the set EXACT — widening it to "any non-hex string" would admit "0x..", which int(_, 16)
    DOES parse, i.e. exactly the two-spellings-of-one-leaf forgery A4 closed."""
    from execnode.code_codec import FIXED_CIDS
    if not isinstance(pre, dict):
        return False, "not an object"
    seen = set()
    for cid, c in pre.items():
        if fixed_names and cid in FIXED_CIDS:
            pass
        elif not (isinstance(cid, str) and cid and cid == cid.lower() and all(ch in "0123456789abcdef" for ch in cid)):
