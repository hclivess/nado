...
> this device" tick uses it (window.nadoProve2) — the node never sees the witness. ~4.4s in node (down from ~22s, a ~5x speedup)
> via a **WASM BLAKE2b** (wasm/blake2b/, ~50x faster hashing), **Montgomery batch inversion**, **cached NTT
> twiddles**, and a **cached periodic LDE**. A WASM Goldilocks field/NTT is the remaining push toward ~2s.
