...
| Network id | `chain_id` (e.g. `betanet-6`) | Bound into every tx — cross-chain replay is impossible. |
...
```jsonc
{
  ...
  "chain_id":    "betanet-6",        // from /status; binds the tx to this network
...
* **Alpha network:** this is testnet-stage (`chain_id: betanet-6`, and it rerolls); treat values as non-final until mainnet.
