...
| [key-rotation.md](key-rotation.md) | **Design proposal** — account abstraction, minimal form: rotate an address's signing key and recover a stolen one via a pre-set recovery key (2-of-2 immediate, single-key delayed + veto), one `proof_authorized` predicate, root/rollback implications |
