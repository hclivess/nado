...
| [key-rotation.md](key-rotation.md) | **Design proposal (Roadmap Track H)** — account authentication as state: authenticators + `signing`/`reconfig` policies (`ID`/`THRESHOLD`, hard-capped), recovery = policy, proof of possession, `config_version`, 1-day pending + cancel/freeze; reviewed against Monad's auth MIP |
