...
**FINAL, from the green run — not an extrapolation.** `tests/test_settle_sparse_fold.py` completed with
`EXIT=0` in **16,773 s (4 h 39 m)** at a peak of **9.4 GB** resident, inside its cgroup. **All ten checks
passed**, including `tampered layer-0 seam rejected by the fold` — the assertion that had never once been
observed passing across six earlier attempts, every one of which died to an OOM-kill, a timeout, or a bug in
the test's own tamper helper. That single assertion is what makes the fold *sound* rather than merely
*working*: without it the fold would accept a doctored inner proof.

The FRI half completes in about an hour; the rest is the composition half, whose comp proof is **W=352
columns over T=8192 rows, LDE'd to N=262144** — and its column commits proceed at about **4.7 columns per
minute**.

For the record, this is the run the Rust port existed to make possible: the same gate under the previous
hybrid prover took 6 h 34 m and ~32 GB, and never finished a clean pass.
